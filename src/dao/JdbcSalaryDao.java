...
public class JdbcSalaryDao implements SalaryDao {
	...
	public List<Salary> findAll(int page, int pageSize, String year,
	...
	public void saveSalarys(List<Salary> salarys) throws Exception {
		 ...
		 * pst=conn.prepareStatement(attendenceSetSql); ResultSet
		 * rs=pst.executeQuery(); AttendenceSet attendenceSet=null;
		 * if(rs.next()){ attendenceSet=new AttendenceSet();
	...
	public void save(Salary salary) throws Exception {
		 ...
		 * pst.setString(3, salary.getTrueName()); pst.setString(4,
		 * salary.getYear()); pst.setString(5, salary.getMonth());
		 * pst.setDouble(6, salary.getBaseSalary()); pst.setDouble(7,
		 * salary.getFinalSalary()); pst.setDouble(8, salary.getInsurance());
		 * pst.setDouble(9, salary.getAttendenceFee()); pst.executeUpdate();
		 * DbUtil.closeConnection();
		 */
	}
	...
	public void deleteById(String id) throws Exception {
		 ...
		 * conn=DbUtil.getConnection(); PreparedStatement
		 * pst=conn.prepareStatement(sql); pst.setString(1, id);
	...
	public List<Salary> saveSalarysByMonth(int page, int pageSize, String year,
		...
		for (int i = 0; i < infos.size(); i++) {
			...
			double finalSalary = info.getBaseSalary() - info.getLateCome()
					* attendenceSet.getLateCome() - info.getEarlyLeave()
					* attendenceSet.getEarlyLeave() - info.getLeave()
					* attendenceSet.getLeave() + info.getOvertime()
					* attendenceSet.getOvertime() - info.getNegletwork()
					* attendenceSet.getNegletwork();

			double finalSal = info.getBaseSalary() - finalSalary
					* insurance.getOld() / 100 - finalSalary
					* insurance.getUnemployment() / 100 - finalSalary
					* insurance.getInjury() / 100 - finalSalary
					* insurance.getBear() / 100 - finalSalary
					* insurance.getMedical() / 100 - finalSalary
					* insurance.getHouse() / 100;
	...
	public List<SalaryStandard> findStand(SalaryStandard salaryStand)
			throws Exception {
		StringBuffer sql = new StringBuffer();
		sql.append("select * from salarystandard where 1=1");
		if (salaryStand.getEmpId() != null
				&& !salaryStand.getEmpId().equals("")) {
			sql.append(" and dempid like'%" + salaryStand.getEmpId() + "%'");
		}
		if (salaryStand.getTrueName() != null
				&& !salaryStand.getTrueName().equals("")) {
